  ...
  return (
    <>
      {schedule && schedule.length > 0 ? (
        <div>Bracket already made</div>
      ) : (
        <>

          <BracketPreview schedule={schedule} intitalSchedule={initialSchedule} />
        </>
      )}
    </>
  )
...
export async function createRoundRobin(roundRobinData: TournamentSchedule, isBo2: boolean, isBo3: boolean, tournanmentName: string)
{
  try {
    ...
    roundRobinData.forEach((round) =>
    {
      round.forEach((match) =>
      {
        ...
        matchesToInsert.push({
          ...
          bo2: isBo2,
          bo3: isBo3,
          tournament_name: tournanmentName,
        })
