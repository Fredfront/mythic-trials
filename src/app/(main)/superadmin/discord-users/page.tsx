...
export default function Component()
{
  ...
  const roleIdToNameMap = new Map<string, string>(
    roles?.map((role) => [ role.id, role.name.toLowerCase() ])
  );
  ...
  const membersWithoutTeams = members.filter(
    (member) =>
      !sanityTeams.some((team) =>
        team.players.some(
          (player) =>
            player.discordName.toLowerCase() === member.user.username.toLowerCase()
        )
      )
  );
                    ...
                    <CardContent>
                      {someMembersHaveRoles && (
                        <Alert variant="warning" className="mb-4">
                          ...
                          <AlertDescription>
                            Some team members are missing the required roles.
                          </AlertDescription>
                        </Alert>
                      )}
                      {allMembersHaveRoles && (
                        <Alert variant="success" className="mb-4">
                          ...
                          <AlertDescription>
                            All team members have the required roles.
                          </AlertDescription>
                        </Alert>
                      )}
                        ...
                        {playersWithMembers.map(({ player, discordMember, memberRoleNames }) => (
                          ...
                          >
                            ...
                            {player.twitchChannel && (
                              <a target='_blank' href={`https://twitch.tv/${player.twitchChannel}`}><Badge variant="secondary" className="ml-auto gap-1">
