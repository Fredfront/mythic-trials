...
export default function RescheduleMatch({
  ...
  const onSubmit = async (data: MatchRescheduleData) => {
    ...
    try {
      ...
      const message = `📢       
      **${isHomeTeam ? homeTeam?.name : awayTeam?.name}** has proposed a new match date! @here
        ...
        <div className="mt-8 p-6 bg-blue-600  rounded-lg shadow-md">
          ...
          <Button
            onClick={async () => {
              ...
              const message = `📢**${isHomeTeam ? homeTeam?.name : awayTeam?.name}** has cancelled the reschedule request. @here`
                    ...
                    <FormItem>
                      ...
                      <FormControl>
                        <input
                          disabled={loading}
                          type="time"
                          className="flex h-10  rounded-md border border-input bg-gray-600 px-3 py-2 text-sm ring-offset-background file:border-0 file:bg-transparent file:text-sm file:font-medium placeholder:text-muted-foreground focus-visible:outline-hidden focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50"
                          {...field}
                        />
                      </FormControl>
                      <FormMessage />
