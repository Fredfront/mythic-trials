...
export function DraggableRoundCarousel({ roundsFromDB }: { roundsFromDB: RoundType[] })
{
  ...
  return (
    <div className="relative w-full overflow-hidden">
      ...
      <Button
        className={`absolute left-2 top-1/2 -translate-y-1/2 z-20 transition-opacity duration-300 ease-in-out bg-gray-800 ${showLeftArrow ? 'opacity-100' : 'opacity-0 pointer-events-none'
          }`}
        size="icon"
        variant="secondary"
        onClick={() => scroll('left')}
      ...
      >
        <ChevronLeft color="#fde047" className="h-6 w-6 " />
      </Button>
      <Button
        className={`absolute right-2 top-1/2 -translate-y-1/2 z-20 transition-opacity duration-300 ease-in-out bg-gray-800 ${showRightArrow ? 'opacity-100' : 'opacity-0 pointer-events-none'
          }`}
        size="icon"
        variant="secondary"
      ...
      >
        <ChevronRight color="#fde047" className="h-6 w-6" />
      </Button>
      {showRightArrow && (
...
type DBRound = {
  ...
  description: string
  type: 'group_stage' | 'playoff' | 'qualifier' | 'signup'
}
